...
def _load_ignore():
    ...
    if not IGNORE_FILE.exists():
        _ignore_cache = []
        return _ignore_cache
    _ignore_cache = [l.strip().lower() for l in IGNORE_FILE.read_text(encoding="utf-8").splitlines() if l.strip() and not l.startswith("#")]
    return _ignore_cache
...
def step_parse(data: SiteData) -> SiteData:
    ...
    data.content_hash = hashlib.md5(html.encode("utf-8", errors="ignore")).hexdigest()
    links = set()
...
def step_detect_language(data: SiteData) -> SiteData:
    if not data.is_online or data.is_ignored or not data.html or len(data.html) < 20:
        return data
    text = re.sub(r'<[^>]+>', '', data.html)[:5000]
    scores = {l: sum(len(p.findall(text)) for p in ps) for l, ps in LANG_PATTERNS.items()}
...
def step_deduplicate(data: SiteData, db) -> SiteData:
    if not data.is_online or data.is_ignored:
        return data
    existing = db.find_by_hash(data.content_hash)
    if existing and existing != data.url:
        data.duplicate_of = existing
    return data


def step_store(data: SiteData, db) -> SiteData:
    if data.is_ignored:
        return data
    kw = dict(title=data.title, status="online" if data.is_online else "offline",
...
def run_pipeline(data: SiteData, db) -> SiteData:
